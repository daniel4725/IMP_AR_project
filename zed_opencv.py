...
def main() :
    ...
    out_send = cv2.VideoWriter('appsrc ! videoconvert ! x264enc tune=zerolatency bitrate=500 speed-preset=superfast ! rtph264pay ! udpsink host=168.254.0.100 port=5005',cv2.CAP_GSTREAMER,0, 20, (1280,360), True)
    ...
    while key != 113 :
        ...
        if err == sl.ERROR_CODE.SUCCESS :
            ...
            image_to_send = np.zeros((image_ocv_left.shape[0],image_ocv_left.shape[1]*2,image_ocv_left.shape[2]-1))
            
            image_to_send[:,:,0] = cv2.hconcat([image_ocv_left[:,:,0] , image_ocv_right[:,:,0]])
            image_to_send[:,:,1] = cv2.hconcat([image_ocv_left[:,:,1] , image_ocv_right[:,:,1]])
            image_to_send[:,:,2] = cv2.hconcat([image_ocv_left[:,:,2] , image_ocv_right[:,:,2]])
            
            imagenorm = cv2.normalize(image_to_send, None, 255, 0, cv2.NORM_MINMAX, cv2.CV_8UC1)
            
            out_send.write(imagenorm)
