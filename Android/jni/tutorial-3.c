...
app_function (void *userdata)
{
  ...
  /* Build pipeline */
  data->pipeline =
          gst_parse_launch("udpsrc port=5005 caps=\"application/x-rtp, media=video, clock-rate=90000, encoding-name=H264, payload=96\" ! rtph264depay ! h264parse ! avdec_h264 ! autovideosink sync=false", &error);
  if (error) {
    gchar *message =
